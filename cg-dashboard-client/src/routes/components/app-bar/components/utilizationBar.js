...
import FromDatePickers from '../../../advance-ui-components/dateTime-picker/components/UtilizationDatePicker';
...
class CustomBar extends Component {
    ...
    onRegularityClick(){
      fetch('https://impact-api.convegenius.com/api/gradeList' , {
         mode :"cors",
         method: "POST",
         headers: {
           'Content-type': 'application/json'
         },
         body: JSON.stringify({ school: this.props.school})
        })
        .then((result) => result.json())
        .then((grade) => { 
           console.log("res");
