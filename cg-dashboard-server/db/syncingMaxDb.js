...
const syncingMaxDb = async (school) => {
    try{
       // console.log(email);
        ...
        for(var i=0;i<syncjson.length;i++){
         ...
         console.log(syncarray);
          ...
          console.log(id);
            ...
            sync.push({device_id: licensejson[index].code, last_sync: syncarray[0].last_sync});
        
            var id = syncarray[syncarray.length-1].device_id;
          //  console.log(id);
              var index = licensejson.findIndex(function(item, i){
                  return item.device_id == id
              });
              ...
              sync.push({device_id: licensejson[index].code, last_sync: syncarray[syncarray.length-1].last_sync});
       return sync;
