...
const improvementOnProjectLevelDb = async (user) => {
 try{
    // console.log(email);
  ...
  for(var i=0;i<intervaljson.length;i++){
    ...
    const preTestScore = await scorespromisify('SELECT  min(`pre_test_score`) AS min, max(`pre_test_score`) AS max from chapter_stats WHERE `student_id` IN(SELECT `id` from student WHERE `center_id` = ? )  AND `chapter_id` IN (SELECT `id` from contentMediumGradeMapping where `contentId` IN (SELECT `id` from content WHERE `id` BETWEEN ? AND ?))',[schooljson[p].id,intervaljson[i].lft,intervaljson[i].rght]);
  ...
  for(var i=0;i<intervaljson.length;i++){
    let scorespromisify = promisify(connection.query).bind(connection);
    const preTestScore = await scorespromisify('SELECT  min(`post_test_score`) AS min, max(`post_test_score`) AS max from chapter_stats WHERE `student_id` IN(SELECT `id` from student WHERE `center_id` = ?)  AND `chapter_id` IN (SELECT `id` from contentMediumGradeMapping where `contentId` IN (SELECT `id` from content WHERE `id` BETWEEN ? AND ?))',[schooljson[p].id,intervaljson[i].lft,intervaljson[i].rght]);
    scorestr = JSON.stringify(preTestScore);
    scorejson = JSON.parse(scorestr);
  ...
  for(var i=0;i<intervaljson.length;i++){
    let scorespromisify = promisify(connection.query).bind(connection);
    const improvementScores = await scorespromisify('SELECT `student_id`, `post_test_score`, `pre_test_score` from chapter_stats WHERE `student_id` IN(SELECT `id` from student WHERE `center_id` = ?)  AND `chapter_id` IN (SELECT `id` from contentMediumGradeMapping where `contentId` IN (SELECT `id` from content WHERE `id` BETWEEN ? AND ?))',[schooljson[p].id,intervaljson[i].lft,intervaljson[i].rght]);
    ...
    let taskpromisify = promisify(connection.query).bind(connection);
      const taskscores = await taskpromisify('SELECT sum(performance) as sum,count(performance) as count from task_stats where student_id IN (SELECT `id` from student WHERE `center_id` =? ) and content_id IN (SELECT `id` from contentMediumGradeMapping where `contentId` IN (SELECT `id` from content WHERE `id` BETWEEN ? AND ?)) GROUP BY performance',[schooljson[p].id,intervaljson[i].lft,intervaljson[i].rght]);
    ...
    if(isNaN(avgImprovement)){
     improvement.push({ Subject: intervaljson[i].defaultName, School: schooljson[p].school_name, Average_Improvement : 0, Average_Task_Score : 0  });

    }
    else{
    improvement.push({ Subject: intervaljson[i].defaultName,School: schooljson[p].school_name, Average_Improvement : avgImprovement, Average_Task_Score : avgTaskScore  });
    }
  }
