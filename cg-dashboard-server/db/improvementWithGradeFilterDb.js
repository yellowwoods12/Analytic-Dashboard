...
const improvementWithGradeFilterDb = async (school,grade) => {
 try{
     console.log(grade);
    ...
    const subjectInterval = await subjectpromisify('Select `defaultName`,`lft`,`rght` from content where type = "SUBJECT"');
...
intervaljson=JSON.parse(intervalstr);
  ...
  for(var i=0;i<intervaljson.length;i++){
    ...
    const preTestScore = await scorespromisify('SELECT  min(`pre_test_score`) AS min, max(`pre_test_score`) AS max from chapter_stats WHERE `student_id` IN(SELECT `id` from student WHERE `center_id` IN(SELECT `id` from fellow WHERE `school_name`= ? ))  AND `chapter_id` IN (SELECT `id` from contentMediumGradeMapping where `contentId` IN (SELECT `id` from content WHERE `id` BETWEEN ? AND ?) AND `grade` = ?)',[school,intervaljson[i].lft,intervaljson[i].rght,grade]);
    ...
    prerow.push( {x: intervaljson[i].defaultName, y : [scorejson[0].min,scorejson[0].max]});
        ...
        prerow.push( {x: intervaljson[i].defaultName, y : [0,0]});

    }
  }
  for(var i=0;i<intervaljson.length;i++){
    let scorespromisify = promisify(connection.query).bind(connection);
    const postTestScore = await scorespromisify('SELECT  min(`post_test_score`) AS min, max(`post_test_score`) AS max from chapter_stats WHERE `student_id` IN(SELECT `id` from student WHERE `center_id` IN(SELECT `id` from fellow WHERE `school_name`= ? ))  AND `chapter_id` IN (SELECT `id` from contentMediumGradeMapping where `contentId` IN (SELECT `id` from content WHERE `id` BETWEEN ? AND ?) AND `grade` = ?)',[school,intervaljson[i].lft,intervaljson[i].rght,grade]);
   ...
   // console.log(scorejson);
    ...
    postrow.push( {x: intervaljson[i].defaultName, y : [0,0]});

  }
}
  for(var i=0;i<intervaljson.length;i++){
    let scorespromisify = promisify(connection.query).bind(connection);
    const improvementScores = await scorespromisify('SELECT `student_id`, `post_test_score`, `pre_test_score` from chapter_stats WHERE `student_id` IN(SELECT `id` from student WHERE `center_id` IN(SELECT `id` from fellow WHERE `school_name`= ?))  AND `chapter_id` IN (SELECT `id` from contentMediumGradeMapping where `contentId` IN (SELECT `id` from content WHERE `id` BETWEEN ? AND ?) AND `grade` = ?)',[school,intervaljson[i].lft,intervaljson[i].rght,grade]);
   ...
   // console.log(scorejson);
    ...
    improvement.push({ Subject: intervaljson[i].defaultName, Average_Improvement :  0, Average_Task_Score : 0  });
    }
