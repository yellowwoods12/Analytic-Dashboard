...
const utilisationWithWeekFilterDb = async (school,from,to) => {
 try{
    // console.log(email);
     let utilisepromisify = promisify(connection.query).bind(connection);
    const utilise = await utilisepromisify('SELECT `device_id`, SUM(datediff(`deletion_date`,`creation_date`)) as `usage`,count(`device_id`) as `logins` from device_student_history where `creation_date` BETWEEN ? AND ? AND student_id IN (SELECT id from student where center_id IN(SELECT `id` from fellow WHERE school_name=?)) group by `device_id` order by `usage` DESC',[from,to,school]);
