...
const regularityWithParameterDb = async (school,parameter_list) => {
    try {
        var outputArray = [];
      ...
      //  console.log(parameter_list);
        ...
        for(var i=0;i<actual_usage.length;i++){
            ...
            var index = expected_usage.findIndex(function(item, i){
                return item.grade == gradeval
            });
        ...
        for(var i=0;i<gradejson.length;i++){
            ...
            const login = await loginpromisify("SELECT `student_id`,count(`student_id`) AS logins FROM user_stats GROUP BY `student_id` HAVING `student_id` IN(SELECT `id` from student WHERE `center_id` IN(SELECT `id` from fellow WHERE `school_name`= ? )AND `grade`= ?)",[school,gradejson[i].grade]);
           // console.log(usage);
        ...
        console.log(gradewiselogin);
        for(var i=0;i<gradejson.length;i++){
            let activepromisify = promisify(connection.query).bind(connection);
            const activestudents = await activepromisify("SELECT distinct(`student_id`) AS active_students FROM user_stats  where `student_id` IN(SELECT `id` from student WHERE `center_id` IN(SELECT `id` from fellow WHERE `school_name`= ? ) AND `grade`= ?)",[school,gradejson[i].grade]);
           // console.log(usage);
