...
const learningWithChapterFilterDb = async (school,chapter) => {
 try{
    // console.log(email);
     let subjectpromisify = promisify(connection.query).bind(connection);
    // let subject = 'Maths';
    const subjectInterval = await subjectpromisify('Select `lft`,`rght` from content where defaultName = ?', chapter);
...
//console.log(intervaljson[0].lft);
    ...
    const testScore = await scorespromisify('SELECT `student_id`,`chapter_id`,`post_test_score`, `pre_test_score` from chapter_stats WHERE `student_id` IN(SELECT `id` from student WHERE `center_id` IN(SELECT `id` from fellow WHERE `school_name`= ? ))  AND `chapter_id` IN (SELECT `id` from contentMediumGradeMapping where `contentId` IN (SELECT `id` from content WHERE `id` BETWEEN ? AND ?))',[school,intervaljson[0].lft,intervaljson[0].rght]);
